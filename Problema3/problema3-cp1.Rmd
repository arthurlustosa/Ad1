...
```{r}
#PCA 
df <- log(ementas.sumarizadas[,2:18])
df2 <- ementas.sumarizadas[,2:18]
autoplot(prcomp(df))


autoplot(prcomp(df), data = iris, colour = 'UF_PROPONENTE', loadings = TRUE)

library(devtools)
install_github('sinhrks/ggfortify')
library(ggfortify); library(ggplot2)
data(iris)
df.pca <- log(ementas.sumarizadas[,2:18])
autoplot(prcomp(df.pca))
autoplot(prcomp(df), data = ementas.sumarizadas[,2:18],
...
km <- kmeans(log(ementas.sumarizadas[,2:18]), centers=5, nstart=10)
km
...
autoplot(kmeans(ementas.sumarizadas[,2:18], 2), data = ementas.sumarizadas)
